...
def get_frontier_limits(
    ...
    :param cov_function: can be one of the HC, SM, GS, IQ_spt
...
def get_frontier_by_risk(returns_df: pd.DataFrame,
    ...
    _port_limits = get_frontier_limits(
    ...
    for target_risk in target_risks_array:
        if target_risk <= min_std:
            ...
            if prev_port_weights is not None and cost is not None:
                weights = port_opt.optimize(
                    'meanVariance', target_std=target_risk, init_weights=init_weights,
                    prev_weights=prev_port_weights["%02dpct" % int(target_risk * 100)]["weights"],
                    cost=cost
                )  # add 10pct as penalty for transaction
            else:
...
def get_mean_variance_space(
    ...
    _rets, _stds, _wgts = get_frontier_by_risk(
        returns_df=returns_df,
    ...
    result_dict['tickers'] = returns_df.columns
    result_dict['cov_function'] = cov_function
    result_dict['freq'] = freq
    result_dict["cov"] = port_opt.covariance
    result_dict["non_psd_cov"] = None if port_opt.is_psd else port_opt.non_psd_covariance
    result_dict["is_psd"] = port_opt.is_psd
    return result_dict
...
def plot_efficient_frontiers(results_dict, prefix="results", plotSize=(10, 8), permno_to_ticker: dict=None):
    ...
    plt.savefig("%s/%s_%s.png" % (prefix, bgn_date_str, end_date_str))
    plt.close()
